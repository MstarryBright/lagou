...
(function () {
    ...
    var arrDbClick = {a: "Link", img: "Img", embed: "Embed"}, uploadInputname = "filedata", arrEntities = {
        ...
        "\u00ae": "&reg;",
        "\u00a9": "&copy;"
    }, regEntities = /[<>"\u00ae\u00a9]/g, Xheditor = function (textarea, options) {
        ...
        this.init = function () {
            ...
            function s(e, t, i, s, a) {
                for (var n, l, r = (i ? i : "") + (s ? s : ""), h = [], c = [], d = 0; o.length > d; d++)n = o[d].r, l = o[d].t, r = r.replace(n, function () {
            ...
            if ("write" === t)e = e.replace(/(<(\/?)(\w+))((?:\s+[\w\-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*((\/?)>)/g, function (e, t, i, s, n, o, l) {
                ...
                if (isWebkit)for (var o = [{
                    ...
                    t: "strike"
                }], l = 0; 2 > l; l++)e = e.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, s), e = e.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/gi, s), e = e.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/gi, s);
            ...
            r && !isNaN(r) && (_jShadow.css("zIndex", parseInt(r, 10) + 1), _jPanel.css("zIndex", parseInt(r, 10) + 2), _jCntLine.css("zIndex", parseInt(r, 10) + 3)), _jPanel.css({
                left: s,
                top: a
